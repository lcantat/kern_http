...
static int tcp_receive_data(struct socket * sk,char * buffer,int    length)
{
	struct msghdr msg;
	struct iovec iov;
	int ret;
...
static int tcp_send_data(struct socket * sk,char * buffer,int length)
{
	struct msghdr msg;
	struct iovec iov;
	int len;
...
static int http_build_reply(char * out_buffer,char * in_buffer,int max_length)
{
    int len;
